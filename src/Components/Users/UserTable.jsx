...
const UserTable = ({ title, path, icon, usertype }) => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`https://dummyjson.com/${usertype}/${id}`)
                    .then(() => {
    ...
    const handleUpdateClick = (userid) => {
        setUserid(userid);
        navigate(`/dashboard/${usertype}/update/${userid}`, { state: { usertype } });
    };
            ...
            <div className="wrapper">
                <div className="users-header flex items-center justify-between">
                    <div className='relative p-2 flex gap-1 items-center'>
                        <h1 className="title lg:text-4xl md:text-3xl text-xl">
                            {title}
                        </h1>
                        {icon}
                    </div>
